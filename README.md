...
1. Разработка ведется в `feature/*` ветках от `dev`.
2. Изменения попадают в `dev` через Pull Request.
3. На PR и push запускается CI (`.github/workflows/ci.yml`): установка зависимостей и сборка.
4. После обновления `dev` запускается автодеплой на DEV-поддомен `div.capital-craft.ru` (`.github/workflows/deploy-dev.yml`).
5. На DEV-поддомене выполняется проверка функционала и интерфейса.
6. После проверки открывается PR `dev -> main`.
7. Merge в `main` выполняется только после прохождения обязательных checks.
8. После обновления `main` запускается автодеплой на основной домен `capital-craft.ru` (`.github/workflows/deploy-prod.yml`).
9. DEV и PROD используют раздельные базы данных, поэтому тестовые изменения не влияют на production-данные.
...
- В рамках согласованного с заказчиком scope адаптация выполнена для mobile и desktop сценариев; отдельная оптимизация промежуточных брейкпойнтов не входила в текущий этап работ.

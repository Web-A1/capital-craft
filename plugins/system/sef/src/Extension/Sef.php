...
final class Sef extends CMSPlugin implements SubscriberInterface
{
    ...
    public function onAfterRoute(AfterRouteEvent $event)
    {
        ...
        // Redirect legacy tag routes to unified Blog tag filter
        try {
            $input = $app->getInput();
            // Case 1: routed to com_tags/tag (old menu still existed)
            if (
                $app->isClient('site')
                && $input->getMethod() === 'GET'
                && $input->getCmd('option') === 'com_tags'
                && $input->getCmd('view') === 'tag'
            ) {
                $idParam = $input->get('id', null);
                $raw = '';
                if (is_array($idParam)) {
                    $raw = (string) ($idParam[0] ?? '');
                } else {
                    $raw = (string) ($idParam ?? '');
                }
                ...
                if ($raw !== '') {
                    $parts = explode(':', $raw, 2);
                    $alias = strtolower($parts[1] ?? '');
                }

                if ($alias !== '') {
                    $menu     = $app->getMenu();
                    $blogItem = $menu->getItems('alias', 'blog', true);
                    $target   = $blogItem ? Route::_('index.php?Itemid=' . (int) $blogItem->id, false) : Route::_('index.php', false);
                    ...
                    $uri->setPort($current->getPort());

                    // Append tag query param
                    $q = $uri->getQuery(true);
                    $q['tag'] = $alias;
                    $uri->setQuery($q);
            ...
            // No-op on redirect failure; continue normal flow
        }

    ...
    if (!empty($faqIds)) {
        ...
        foreach ($faqItems as &$it) {
            if (isset($tagsById[$it["id"]])) {
                $tagList = array_values($tagsById[$it["id"]]);
                $it["tags"] = $tagList;
                $it["primary_tag"] = $tagList[0] ?? null;
            }
        }
        unset($it);
    }
    ...
    if (!empty($faqItems) && !empty($faqAllTags)) {
        ...
        if ($selectedAlias) {
            usort($orderedTags, function ($a, $b) use ($selectedAlias) {
                $aa = strtolower($a->alias);
                $bb = strtolower($b->alias);
                if ($aa === $selectedAlias && $bb !== $selectedAlias) {
                    return -1;
                }
                if ($bb === $selectedAlias && $aa !== $selectedAlias) {
                    return 1;
                }

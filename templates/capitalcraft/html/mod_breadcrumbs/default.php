...
if ($option === "com_tags" && $view === "tag") {
    ...
    // Параметр id приходит в com_tags как массив, даже для одиночного тега
    $idParam = $input->get("id", [], "array");
    $idParam = (string) ($idParam[0] ?? "");
    $tagTitle = "";
    $db = Factory::getDbo();
    $q = $db->getQuery(true)->select($db->quoteName("title"))->from($db->quoteName("#__tags"));
    ...
    // Драйверы базы и нестандартные расширения могут вернуть массивы или
    ...
    if ($home) {
        $custom[] = (object) [
            "name" => Text::_("MOD_BREADCRUMBS_HOME"),
            "link" => Route::_("index.php?Itemid=" . $home->id),
        ];
    }
    if ($blogItem) {

  ...
  "templates/capitalcraft/js/global/burger.js"() {
    ...
    initBurger = () => {
      ...
      document.addEventListener("keydown", (e2) => {
        if (e2.key === "Escape" && document.body.classList.contains("menu-open")) {
          closeMenu();
        }
      });
      mobileNav.addEventListener("click", (e2) => {
...
function objectIncludes(b, a2) {
  if (a2 === b) return true;
  const arrA = Array.isArray(a2), arrB = Array.isArray(b);
  let i2;
  if (arrA && arrB) {
    if (a2.length != b.length) return false;
    for (i2 = 0; i2 < a2.length; i2++) if (!objectIncludes(a2[i2], b[i2])) return false;
    return true;
  }
  if (arrA != arrB) return false;
  if (a2 && b && typeof a2 === "object" && typeof b === "object") {
    const dateA = a2 instanceof Date, dateB = b instanceof Date;
    if (dateA && dateB) return a2.getTime() == b.getTime();
    if (dateA != dateB) return false;
    const regexpA = a2 instanceof RegExp, regexpB = b instanceof RegExp;
    if (regexpA && regexpB) return a2.toString() == b.toString();
    if (regexpA != regexpB) return false;
    const keys = Object.keys(a2);
    for (i2 = 0; i2 < keys.length; i2++) if (!Object.prototype.hasOwnProperty.call(b, keys[i2])) return false;
    for (i2 = 0; i2 < keys.length; i2++) if (!objectIncludes(b[keys[i2]], a2[keys[i2]])) return false;
    return true;

...
class CapitalcraftRelatedHelper
{
    ...
    public static function getRelatedForArticle($article, array $tagIds): array
    {
        ...
        $languageList = implode(
            ",",
            array_map(function ($lang) use ($db) {
        ...
        $articleQuery = $db
            ...
            ->group("c.id, c.title, c.alias, c.catid, c.language, c.publish_up, c.introtext, c.fulltext")
        ...
        $faqQuery = $db
            ...
            ->group("c.id, c.title, c.alias, c.catid, c.language, c.publish_up, c.introtext, c.fulltext")
            ->order("c.publish_up DESC");
        // FAQ: выводим все совпадения (ограничение на фронте через верстку/скролл)
        ...
        foreach ($rows as $row) {
            $itemTagIds = array_values(array_filter(array_map("intval", explode(",", (string) ($row->tag_ids ?? "")))));
            if (!empty($itemTagIds)) {
            ...
            if ((int) $row->sort_group === 1) {
                ...
                $excerpt = $plain !== "" ? HTMLHelper::_("string.truncate", $plain, 240, true, false) : "";
                ...
                $excerpt = $answerText !== "" ? HTMLHelper::_("string.truncate", $answerText, 200, true, false) : "";

                $fallbackAliases = array_values(array_filter(explode(",", (string) ($row->tag_aliases ?? ""))));
    ...
    protected static function resolveFaqLinkAlias(
        array $itemTagIds,
        array $articleTagAliasMap,
        array $fallbackAliases,
    ): string {

...
compile_less_file() {
    ...
    if [[ "$file" == *"/_variables.less" || "$file" == *"/_buttons.less" || "$file" == *"/_header.less" || "$file" == *"/_footer.less" || "$file" == *"/_modal.less" || "$file" == *"/_scroll-top.less" || "$file" == *"/_breadcrumbs.less" ]]; then
        ...
        npm run less:faq
    elif [[ "$file" == *"/legal.less" || "$file" == *"/pages/legal/"* ]]; then
        echo "    Legal раздел - компилирую legal.css"
        npm run less:legal
        ...
        if [[ "$LESS_CHANGED" == true ]]; then
            ...
            if [[ "$LESS_ALL_BUILT" == true ]]; then
                ...
                git add templates/capitalcraft/css/home.css \
                        ...
                        templates/capitalcraft/css/legal.css || true
            fi
            ...
            if echo "$PROCESSED_FILES" | grep -Eq "pages/legal/|/legal.less"; then
                echo "   └─ Добавляю CSS: legal.css"
                git add templates/capitalcraft/css/legal.css
            fi
            echo ""
        fi

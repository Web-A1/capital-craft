...
<div class="container-popup">
    ...
    <form action="<?php echo Route::_(
        'index.php?option=com_categories&view=categories&layout=modal&tmpl=component&function=' .
            $function .
            '&' .
                            ...
                            <td class="text-center">
                                <span class="tbody-icon">
                                    <span class="<?php echo $iconStates[
                                        $this->escape($item->published)
                                    ]; ?>" aria-hidden="true"></span>
                                </span>
                            </td>
                            <th scope="row">
                                <?php $attribs =
                                    'data-content-select data-content-type="' .
                                    $extension .
                                    '.category"' .
                                    ' data-id="' .
                                    $item->id .
                                    ...
                                    '"' .
                                    ' data-html="' .
                                    $this->escape($itemHtml) .
                                    '"'; ?>
                                <?php echo LayoutHelper::render('joomla.html.treeprefix', ['level' => $item->level]); ?>
                                <a href="javascript:void(0)" <?php echo $attribs; ?> onclick="if (window.parent && !window.parent.JoomlaExpectingPostMessage) window.parent.<?php echo $this->escape(
     $function,
 ); ?>('<?php echo $item->id; ?>', '<?php echo $this->escape(
    addslashes($item->title),
); ?>', null, '<?php echo $this->escape(
    RouteHelper::getCategoryRoute($item->id, $item->language),

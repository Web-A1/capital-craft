...
class Joomla extends BasePlatform
{
    ...
    public function get_stock_directories()
    {
        ...
        if (empty($stock_directories)) {
            ...
            $stock_directories['[DEFAULT_OUTPUT]'] =
                $this->get_site_root() . '/administrator/components/com_akeebabackup/backup';
    ...
    public function get_administrator_emails()
    {
        ...
        foreach ($groups as $gid) {
            ...
            array_walk($uids, function ($uid, $index) use (&$mails) {
                $mails[] = JoomlaFactory::getContainer()->get(UserFactoryInterface::class)->loadUserById($uid)->email;
            });
        }
    ...
    public function send_email($to, $subject, $body, $attachFile = null)
    {
        ...
        if (!is_object($mailer)) {
            Factory::getLog()->log(
                LogLevel::WARNING,
                "Could not send email to $to - Joomla! cannot send e-mails. Please check your From EMail and From Name fields in Global Configuration.",
            );
        ...
        try {
            ...
            Factory::getLog()->log(
                LogLevel::WARNING,
                "Could not send email to $to - Problem setting up the email. Joomla! reports error: " .
                    $e->getMessage(),
            );
        ...
        try {
            if (!empty($attachFile)) {
                ...
                if (!file_exists($attachFile) || !(is_file($attachFile) || is_link($attachFile))) {
                    Factory::getLog()->log(
                        LogLevel::WARNING,
                        "The file does not exist, or it's not a file; no email sent",
                    );
                ...
                if ($filesize) {
                    ...
                    if ($availableRAM < 2.5 * $filesize) {
                        Factory::getLog()->log(
                            LogLevel::WARNING,
                            'The file is too big to be sent by email. Please use a smaller Part Size for Split Archives setting.',
                        );
                        Factory::getLog()->log(
                            LogLevel::DEBUG,
                            "Memory limit $totalRAM bytes -- Used memory $usedRAM bytes -- File size $filesize -- Attachment requires approx. " .
                                2.5 * $filesize .
                                ' bytes',
                        );
                    ...
                    Factory::getLog()->log(
                        LogLevel::WARNING,
                        "Your server fails to report the file size of $attachFile. If the backup crashes, please use a smaller Part Size for Split Archives setting",
                    );
                }
            ...
            Factory::getLog()->log(
                LogLevel::WARNING,
                "Could not send email to $to - Problem attaching file. Joomla! reports error: " . $e->getMessage(),
            );
    ...
    public function apply_quirk_definitions()
    {
        ...
        $configurationCheck->addConfigurationCheckDefinition(
            '013',
            'critical',
            'COM_AKEEBABACKUP_CPANEL_WARNING_Q013',
            [Joomla::class, 'quirk_013'],
        );
        $configurationCheck->addConfigurationCheckDefinition(
            '015',
            'critical',
            'COM_AKEEBABACKUP_CPANEL_WARNING_Q015',
    ...
    private function getDownloadId(string $package = 'pkg_akeebabackup')
    {
        ...
        $query = (method_exists($db, 'createQuery') ? $db->createQuery() : $db->getQuery(true))
            ...
            ->innerJoin(
                $db->qn('#__update_sites_extensions') .
                    ' ' .
                    $db->qn('ue') .
                    ' ON(' .
                    $db->qn('ue.update_site_id') .
                    ' = ' .

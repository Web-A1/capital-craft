...
$engineForProfile = array_key_exists($record['profile_id'], $this->enginesPerProfile)
    ? $this->enginesPerProfile[$record['profile_id']]
    : 'none';
$showUploadRemote =
    $this->permissions['backup'] &&
    $archiveExists &&
    !$showManageRemote &&
    $engineForProfile != 'none' &&
    $record['meta'] != 'obsolete' &&
    AKEEBABACKUP_PRO == 1;
	...
	<?php if ($showManageRemote): ?>
		<div style="padding-bottom: 3pt;">
			<button type="button"
					...
					data-management="index.php?option=com_akeebabackup&view=Remotefiles&tmpl=component&task=listactions&id=<?= (int) $record[
         'id'
     ] ?>"
...
<div>
	...
	<?php if ($showViewLog): ?>
        <a class="akeebabackup-viewlog-button btn btn-outline-dark btn-small text-decoration-none me-2 mb-2 akeebaCommentPopover"
           ...
           href="index.php?option=com_akeebabackup&view=Log&tag=<?= $this->escape($record['tag']) ?>.<?= $this->escape(
    $record['backupid'],
) ?>&profileid=<?= (int) $record['profile_id'] ?>"

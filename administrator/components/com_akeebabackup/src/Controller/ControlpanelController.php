...
class ControlpanelController extends BaseController
{
    ...
    public function SwitchProfile($cachable = false, $urlparams = [])
    {
        ...
        if (!is_numeric($newProfile) || $newProfile <= 0) {
            $this->setRedirect(
                Uri::base() . 'index.php?option=com_akeebabackup',
                Text::_('COM_AKEEBABACKUP_CPANEL_PROFILE_SWITCH_ERROR'),
                'error',
            );
    ...
    public function resetSecretWord($cachable = false, $urlparams = [])
    {
        ...
        $this->app->bootComponent('com_akeebabackup')->getComponentParametersService()->save($params);

...
class PushMessages implements PushMessagesInterface
{
    ...
    public function message($subject, $details = null)
    {
        ...
        foreach ($this->getWebPushUsers() as $uid) {
            ...
            if ($failed > 0) {
                $logger->debug(
                    sprintf(
                        '[Web Push] Partially notified user ID %d on %d subscription(s), %d of which failed',
                        $uid,
                        $total,
                        $failed,
                    ),
                ...
                $logger->debug(
                    sprintf('[Web Push] Successfully notified user ID %d on %d subscription(s)', $uid, $total),
                );
            }
        }
    }
    ...
    private function getWebPushUsers(): array
    {
        ...
        try {
            ...
            self::$webPushUsers = array_filter($db->setQuery($query)->loadColumn() ?: [], function ($uid) {
                /** @var User|null; $user */
                $user = Factory::getContainer()->get(UserFactoryInterface::class)->loadUserById($uid);

                return $user instanceof User && $user->authorise('core.manage', 'com_akeebabackup');
            });
        } catch (\Exception $e) {

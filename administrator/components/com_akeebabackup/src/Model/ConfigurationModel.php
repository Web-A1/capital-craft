...
class ConfigurationModel extends BaseModel
{
    ...
    public function saveEngineConfig(): void
    {
        ...
        if (array_key_exists('akeeba.basic.output_directory', $data)) {
            ...
            if ($check == $folder) {
                ...
                $data['akeeba.basic.output_directory'] = Factory::getFilesystemTools()->rebaseFolderToStockDirs(
                    $data['akeeba.basic.output_directory'],
                );
            }
        }
    ...
    public function testFTP(): void
    {
        ...
        $realdir =
            substr($config['directory'], -1) == '/'
                ? substr($config['directory'], 0, strlen($config['directory']) - 1)
                : $config['directory'];
    ...
    public function dpeOuthOpen(): void
    {
        ...
        $params['callbackURI'] =
            rtrim(Uri::base(), '/') .
            '/index.php?option=com_akeebabackup&view=Configuration&task=dpecustomapiraw&engine=' .
            $engine;

...
class StatisticsModel extends ListModel
{
    ...
    public function __construct($config = [], MVCFactoryInterface $factory = null)
    {
        if (empty($config['filter_fields'])) {
            $config['filter_fields'] = ['search', 'from', 'to', 'origin', 'profile', 'frozen'];
        }
    ...
    public function notifyFailed()
    {
        ...
        if (empty($superAdmins)) {
            return [
                'message' => [
                    'Failed backup(s) detected, but there are no configured Super Administrators to receive notifications',
                ],
                'result' => false,
            ];
        }
        ...
        return [
            'message' => [
                sprintf(
                    ...
                    is_array($failed) || $failed instanceof \Countable ? count($failed) : 0,
                ),
                sprintf('Sent %s notifications', count($superAdmins)),
            ],
            'result' => false,
        ];
    }
    ...
    protected function getListQuery()
    {
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where($db->quoteName('id') . ' = :id')->bind(':id', $ids, ParameterType::INTEGER);
            } else {
                $search = '%' . $search . '%';
                $query->where($db->qn('description') . ' LIKE :search')->bind(':search', $search);
            }
        }
        ...
        if (!empty($from) && !empty($to)) {
            ...
            $query
                ->where($db->qn('backupstart') . ' BETWEEN :from AND :to')
                ->bind(':from', $from, ParameterType::STRING)
                ->bind(':to', $to, ParameterType::STRING);
        } elseif (!empty($from)) {
            $from = (clone JoomlaFactory::getDate($from))->toSql();
            $query->where($db->qn('backupstart') . ' >= :from')->bind(':from', $from, ParameterType::STRING);
        } elseif (!empty($to)) {
            $to = (clone JoomlaFactory::getDate($to))->toSql();
            $query->where($db->qn('backupstart') . ' <= :to')->bind(':to', $to, ParameterType::STRING);
        }
        ...
        if (is_string($origin) && !empty($origin)) {
            $query->where($db->qn('origin') . ' = :origin')->bind(':origin', $origin, ParameterType::STRING);
        }
        ...
        if (is_numeric($profile)) {
            $query->where($db->qn('profile_id') . ' = :profile')->bind(':profile', $profile, ParameterType::INTEGER);
        }
        ...
        if (is_numeric($frozen)) {
            ...
            $query->where($db->qn('frozen') . ' = :frozen')->bind(':frozen', $frozen, ParameterType::INTEGER);
        }

...
class BackupModel extends BaseDatabaseModel
{
    ...
    public function convertLogFiles($timeOut = 10)
    {
        ...
        $killFiles = ['akeeba.log', 'akeeba.backend.log', 'akeeba.frontend.log', 'akeeba.cli.log', 'akeeba.json.log'];
    ...
    public function getDefaultDescription(string $format = ''): string
    {
        ...
        $forcedTZ = Platform::getInstance()->get_platform_configuration_option(
    ...
    public function startBackup(array $overrides = []): array
    {
        ...
        $profile = JoomlaFactory::getApplication()
            ->getSession()
            ->get('akeebabackup.profile', defined('AKEEBA_PROFILE') ? AKEEBA_PROFILE : 1);
        ...
        if (!Factory::getConfigurationChecks()->getShortStatus()) {
            ...
            foreach ($configChecks as $checkItem) {
                ...
                return [
                    ...
                    'Substep' => '',
                    'Error' =>
                        'Failed configuration check Q' .
                        $checkItem['code'] .
                        ': ' .
                        $checkItem['description'] .
                        '. Please refer to https://www.akeeba.com/documentation/warnings/q' .
                        $checkItem['code'] .
                        '.html for more information and troubleshooting instructions.',
                    'Warnings' => [],
                    'Progress' => 0,
                ];
            }
    ...
    public function stepBackup($requireBackupId = true)
    {
        ...
        if (!empty($ret_array['Error']) || $ret_array['HasRun'] == 1) {
            ...
            if (
                JoomlaFactory::getApplication()->isClient('administrator') &&
                $config->get('akeeba.advanced.autoresume', 1)
            ) {

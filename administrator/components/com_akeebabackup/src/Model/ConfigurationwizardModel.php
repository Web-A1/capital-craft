...
class ConfigurationwizardModel extends BaseDatabaseModel
{
    ...
    {
        try {
            $default = JoomlaFactory::getApplication()->input->get(
                $property,
                $default,
                is_array($default) ? 'array' : 'raw',
            );
        } catch (Exception $e) {
        }
    ...
    {
        $seconds = JoomlaFactory::getApplication()->input->get('seconds', '0.5', 'float');
    ...
    private function applyminexec(): array
    {
        ...
        $minexec = JoomlaFactory::getApplication()->input->get('minexec', 2.0, 'float');
    ...
    {
        $seconds = JoomlaFactory::getApplication()->input->get('seconds', 30, 'int');
        $timer = Factory::getTimer();
    ...
    {
        // Get the user parameters
        $maxexec = JoomlaFactory::getApplication()->input->get('seconds', 2, 'int');
    ...
    private function flush(): array
    {
        ...
        $paramsService = JoomlaFactory::getApplication()

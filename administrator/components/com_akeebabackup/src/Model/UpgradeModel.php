...
class UpgradeModel extends BaseModel implements DatabaseAwareInterface
{
     ...
     * @var   array
     */
    private const PRO_CRITERIA = [['const', 'AKEEBABACKUP_PRO'], ['const', 'AKEEBABACKUP_INSTALLATION_PRO']];
    ...
    private const REMOVE_FROM_ALL_VERSIONS = [
        'files' => [
            ...
            JPATH_ADMINISTRATOR .
            'administrator/components/com_akeebabackup/vendor/akeeba/engine/Postproc/idrivesync.json',
            JPATH_ADMINISTRATOR .
            'administrator/components/com_akeebabackup/vendor/akeeba/engine/Postproc/Idrivesync.php',
            JPATH_ADMINISTRATOR .
            'administrator/components/com_akeebabackup/vendor/akeeba/engine/Postproc/Connector/Idrivesync.php',
    ...
    private const ENABLE_EXTENSIONS = ['plg_quickicon_akeebabackup', 'plg_system_backuponupdate'];
    ...
    public function getExtensionId(string $extension): ?int
    {
        ...
        foreach ($criteria as $key => $value) {
            ...
            $query->where($db->qn($key) . ' = :' . $key)->bind(':' . $key, ${$varName}, $type);
        }

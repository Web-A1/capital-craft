...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        $urls = [
            ...
            'dpeauthopen' => addslashes(
                'index.php?option=com_akeebabackup&view=Configuration&task=dpeoauthopen&format=raw',
            ),
            'dpecustomapi' => addslashes(
                'index.php?option=com_akeebabackup&view=Configuration&task=dpecustomapi&format=raw',
            ),
        ];
        ...
        $this->getDocument()->addScriptOptions(
            'akeebabackup.Configuration.GUIData',
            json_decode(Factory::getEngineParamsProvider()->getJsonGuiDefinition(), true),
        );
    ...
    protected function addToolbar(): void
    {
        ...
        $saveGroup->configure(function (Toolbar $childBar) {
            $childBar->save('save');
        ...
        $toolbar
            ->link(Text::_('COM_AKEEBABACKUP_CONFWIZ'), 'index.php?option=com_akeebabackup&view=Configurationwizard')
            ->icon('fa fa-bolt');

        if (AKEEBABACKUP_PRO) {
            $toolbar
                ->link(Text::_('COM_AKEEBABACKUP_SCHEDULE'), 'index.php?option=com_akeebabackup&view=Schedule')
                ->icon('fa fa-calendar');
        }

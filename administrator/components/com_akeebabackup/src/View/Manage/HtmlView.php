...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        $this->getDocument()->getWebAssetManager()->useScript('com_akeebabackup.manage');
        ...
        $this->getDocument()
            ...
            ->addScriptOptions(
                'akeebabackup.Manage.downloadURL',
                Route::_(
                    'index.php?option=com_akeebabackup&task=Manage.download&' .
                        $app->getSession()->getFormToken() .
                        '=1',
                    false,
                ),
            );
    ...
    private function addToolbar(): void
    {
        ...
        if (AKEEBABACKUP_PRO) {
            $toolbar
                ->linkButton('discover', 'COM_AKEEBABACKUP_DISCOVER')
        ...
        if ($permissions['configure']) {
            ...
            if ($permissions['configure'] && AKEEBABACKUP_PRO) {
                $childBar
                    ->standardButton('restore', 'COM_AKEEBABACKUP_BUADMIN_LABEL_RESTORE', 'restore.main')
            ...
            $childBar
                ->delete('manage.deletefiles')
                ->icon('fa fa-broom')
                ->text('COM_AKEEBABACKUP_BUADMIN_LABEL_DELETEFILES')
                ->message('JGLOBAL_CONFIRM_DELETE')
                ->listCheck(true);

            $childBar->delete('manage.delete')->message('JGLOBAL_CONFIRM_DELETE')->listCheck(true);
        }

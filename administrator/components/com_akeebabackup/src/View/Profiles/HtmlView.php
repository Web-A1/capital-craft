...
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolbar(): void
    {
        ...
        $toolbar->standardButton('copy', 'COM_AKEEBABACKUP_LBL_BATCH_COPY', 'profiles.copy')->listCheck(true);
        ...
        if ($user->authorise('akeebabackup.configure', 'com_akeebabackup')) {
            ...
            $childBar
                ->unpublish('profiles.unpublish')
                ->icon('fa fa-times-circle')
                ->text('COM_AKEEBABACKUP_PROFILES_BTN_UNPUBLISH')
                ->listCheck(true);

            $childBar
                ->standardButton('reset', 'COM_AKEEBABACKUP_PROFILES_BTN_RESET', 'profiles.reset')
                ->icon('fa fa-radiation')
                ->listCheck(true);

            $childBar->delete('profiles.delete')->message('JGLOBAL_CONFIRM_DELETE')->listCheck(true);
        }

        $toolbar
            ->linkButton('import', 'COM_AKEEBABACKUP_PROFILES_IMPORT', '')
            ->icon('fa fa-upload')
            ->attributes([
        ...
        if (
            $user->authorise('core.admin', 'com_akeebabackup') ||
            $user->authorise('core.options', 'com_akeebabackup')
        ) {
        ...
        $toolbar->help(
            null,
            false,
            'https://www.akeeba.com/documentation/akeeba-backup-joomla/using-basic-operations.html#profiles-management',
        );
    }
}
